  ...
  onStateChange() {
    if (this.monitoringState === MONITORING_STATE.ERROR) {
      this.loader.setMessage(
        $localize`:@@message system error:The system is not working correctly, you can't make changes in the configuration!`
      )
      ...
      this.loader.setMessage(
        $localize`:@@message system not ready:The system is not ready, you can't make changes in the configuration!`
      )
    } else {
      this.loader.clearMessage()
    }
