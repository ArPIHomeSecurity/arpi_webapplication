...
export class UserService {
  ...
  generateRegistrationCode(userId: number, expiry: number): Observable<any> {
    ...
    return this.http.get("/api/user/" + userId + "/registration_code", {
      params
    })
  }
  ...
  setPublicKey(userId: number, publicKey: string): Observable<boolean> {
    return this.http
      .put("/api/user/" + userId + "/ssh_key", { publicKey })
      .pipe(map((res: any) => res))
  }
  ...
  hasMCPToken(userId: number): Observable<boolean> {
    return this.http
      .get("/api/user/" + userId + "/mcp_token")
      .pipe(map((res: any) => res.has_mcp_token))
  }
